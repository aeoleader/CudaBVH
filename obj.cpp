...
void obj::buildVBOs(){
	...
	for(int k = 0; k<faces.size(); k++){
		...
		//if(0==0){
			...
			Vector4 p0 = points[face[0]];
			
			for(int i=2; i<face.size(); i++){
				Vector4 p1 = points[face[i-1]];
				Vector4 p2 = points[face[i]];
				...
				if(genNormals==false){
					...
					Vector3 a = Vector3(p1[0], p1[1], p1[2]) - Vector3(p0[0], p0[1], p0[2]);
					Vector3 b = Vector3(p2[0], p2[1], p2[2]) - Vector3(p0[0], p0[1], p0[2]);
...
bool obj::isConvex(vector<int> face){
	...
	Vector3 a = Vector3(points[face[0]][0], points[face[0]][1], points[face[0]][2]) - Vector3(points[face[k]][0], points[face[k]][1], points[face[k]][2]);
	Vector3 b = Vector3(points[face[0]][0], points[face[0]][1], points[face[0]][2]) - Vector3(points[face[1]][0], points[face[1]][1], points[face[1]][2]);
	Vector3 n = normalize(cross(a,b));

	for(int i=2; i<face.size(); i++){
		Vector3 c = Vector3(points[face[i-1]][0], points[face[i-1]][1], points[face[i-1]][2]) - Vector3(points[face[i-2]][0], points[face[i-2]][1], points[face[i-2]][2]);
		Vector3 d = Vector3(points[face[i-1]][0], points[face[i-1]][1], points[face[i-1]][2]) - Vector3(points[face[i]][0], points[face[i]][1], points[face[i]][2]);
		Vector3 m = normalize(cross(c,d));
	...
	Vector3 c = Vector3(points[face[k]][0], points[face[k]][1], points[face[k]][2]) - Vector3(points[face[k-1]][0], points[face[k-1]][1], points[face[k-1]][2]);
	Vector3 d = Vector3(points[face[k]][0], points[face[k]][1], points[face[k]][2]) - Vector3(points[face[0]][0], points[face[0]][1], points[face[0]][2]);
	Vector3 m = normalize(cross(c,d));
	if((abs(m[0] - n[0]) > EPSILON) || (abs(m[1] - n[1]) > EPSILON) || (abs(m[2] - n[2]) > EPSILON)){	
		return false;
	}
...
void obj::addPoint(Vector3 point){
    ...
    points.push_back(Vector4(point[0], point[1], point[2], 1));
...
void obj::addNormal(Vector3 normal){
	normals.push_back(Vector4(normal[0], normal[1], normal[2], 1));
}

void obj::addTextureCoord(Vector3 texcoord){
	texturecoords.push_back(Vector4(texcoord[0], texcoord[1], texcoord[2], 1));
}
...
void obj::recenter(){
	Vector3 center = Vector3((xmax+xmin)/2,ymin,(zmax+zmin)/2);
...
void obj::setColor(Vector3 newColor){
	cbosize = ibosize*3;
...
vector<Vector4>* obj::getNormals(){
    return &normals;
}

vector<Vector4>* obj::getTextureCoords(){
